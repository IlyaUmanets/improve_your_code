...
RSpec.describe ImproveYourCode::Context::MethodContext do
  let(:method_context) { described_class.new(exp, nil) }

  describe '#matches?' do
    let(:exp) { instance_double('ImproveYourCode::AST::SexpExtensions::ModuleNode').as_null_object }
  ...
  describe '#default_assignments' do
    def assignments_from(src)
      exp = ImproveYourCode::Source::SourceCode.from(src).syntax_tree
      ctx = ImproveYourCode::Context::MethodContext.new(exp, nil)
