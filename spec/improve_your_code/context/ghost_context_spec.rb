...
require_lib 'improve_your_code/context/code_context'
require_lib 'improve_your_code/context/ghost_context'

RSpec.describe ImproveYourCode::Context::GhostContext do
  let(:exp) { instance_double('ImproveYourCode::AST::Node') }
  let(:parent) { ImproveYourCode::Context::CodeContext.new(exp) }
  ...
  describe '#append_child_context' do
    ...
    it 'appends the child to the grandparent context' do
      child = ImproveYourCode::Context::CodeContext.new(sexp(:foo))
