...
require_lib 'improve_your_code/context/method_context'
require_lib 'improve_your_code/context/module_context'

RSpec.describe ImproveYourCode::Context::CodeContext do
  describe '#full_name' do
    let(:ctx)       { described_class.new(exp) }
    let(:exp)       { instance_double('ImproveYourCode::AST::SexpExtensions::ModuleNode') }
    ...
    context 'when there is an outer' do
      ...
      let(:outer)      { described_class.new(instance_double('ImproveYourCode::AST::Node')) }
  ...
  describe '#each_node' do
    context 'with an empty module' do
      let(:ctx) do
        ...
        ast = ImproveYourCode::Source::SourceCode.from(src).syntax_tree
  ...
  describe '#config_for' do
    let(:src) do
      <<-EOS
        # :improve_your_code:DuplicateMethodCall: { allow_calls: [ puts ] }')
    ...
    let(:expression) { ImproveYourCode::Source::SourceCode.from(src).syntax_tree }
    let(:outer) { nil }
    let(:context) { described_class.new(expression) }
    let(:sniffer) { class_double('ImproveYourCode::SmellDetectors::BaseDetector') }
  ...
  describe '#register_with_parent' do
    let(:context) { described_class.new(instance_double('ImproveYourCode::AST::Node')) }
    let(:first_child) { described_class.new(instance_double('ImproveYourCode::AST::Node')) }
    let(:second_child) { described_class.new(instance_double('ImproveYourCode::AST::Node')) }
