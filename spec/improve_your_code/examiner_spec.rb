...
require_lib 'improve_your_code/logging_error_handler'
...
RSpec.describe ImproveYourCode::Examiner do
  ...
  context 'with a partially masked smelly File' do
    ...
    let(:path) { CONFIG_PATH.join('partial_mask.improve_your_code') }
    let(:expected_first_smell) { 'UncommunicativeVariableName' }
  ...
  describe '#smells' do
    it 'returns the detected smell warnings' do
      ...
      expect(smell).to be_a(ImproveYourCode::SmellWarning)
    ...
    context 'with an incomprehensible source that causes the detectors to crash' do
      let(:source) { 'class C; def does_crash_improve_your_code; end; end' }

      let(:examiner) do
        detector_repository = instance_double 'ImproveYourCode::DetectorRepository'
        allow(detector_repository).to receive(:examine) do
          raise ArgumentError, 'Looks like bad source'
        end
        class_double('ImproveYourCode::DetectorRepository').as_stubbed_const
        allow(ImproveYourCode::DetectorRepository).to receive(:eligible_smell_types)
        allow(ImproveYourCode::DetectorRepository).to receive(:new).and_return detector_repository

        described_class.new source
      end

      it 'raises an incomprehensible source error' do
        expect { examiner.smells }.to raise_error ImproveYourCode::Errors::IncomprehensibleSourceError
      end
      ...
      it 'explains what to do' do
        explanation = 'Please double check your ImproveYourCode configuration'
  ...
  context 'when the source causes the source buffer to crash' do
    ...
    context 'when the error handler handles the error' do
      let(:handler) { instance_double(ImproveYourCode::LoggingErrorHandler, handle: true) }
      ...
      it 'passes the wrapped error to the handler' do
        examiner.smells
        expect(handler).to have_received(:handle).with(ImproveYourCode::Errors::IncomprehensibleSourceError)
      end
    end
  end
  ...
  context 'with a source that triggers an encoding error' do
    ...
    it 'raises an encoding error when asked for smells' do
      expect { examiner.smells }.to raise_error ImproveYourCode::Errors::EncodingError
    end

    it 'explains the origin of the error' do
      message = "Source 'string' cannot be processed by ImproveYourCode due to an encoding error in the source file."
  ...
  describe 'bad comment config' do
    ...
    context 'with an unknown smell detector' do
      let(:source) do
        <<-EOS
          # :improve_your_code:DoesNotExist
          def alfa; end
        EOS
      end

      it 'raises a bad detector name error' do
        expect { examiner.smells }.to raise_error ImproveYourCode::Errors::BadDetectorInCommentError
      end
    ...
    context 'with garbage in detector config' do
      let(:source) do
        <<-EOS
          # :improve_your_code:UncommunicativeMethodName { thats: a: bad: config }
          def alfa; end
        EOS
      end

      it 'raises a garbarge configuration error' do
        expect { examiner.smells }.to raise_error ImproveYourCode::Errors::GarbageDetectorConfigurationInCommentError
      end
