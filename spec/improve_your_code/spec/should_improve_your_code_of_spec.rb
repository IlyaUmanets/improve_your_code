...
require_lib 'improve_your_code/spec'
require 'active_support/core_ext/hash/except'

RSpec.describe ImproveYourCode::Spec::ShouldImproveYourCodeOf do
  describe 'smell type selection' do
    ...
    it 'reports duplicate calls by smell type' do
      expect(ruby).to improve_your_code_of(:DuplicateMethodCall)
    end

    it 'reports duplicate calls by smell detector class' do
      expect(ruby).to improve_your_code_of(ImproveYourCode::SmellDetectors::DuplicateMethodCall)
    end

    it 'does not report any feature envy' do
      expect(ruby).not_to improve_your_code_of(:FeatureEnvy)
    end
  end
    ...
    context 'when no smell_type is matching' do
      ...
      it 'sets the proper error message' do
        ...
        expect(falsey_matcher.failure_message).to\
          match('Expected string to improve_your_code of FeatureEnvy, but it didn\'t')
      end
      ...
      it 'sets the proper error message when negated' do
        ...
        expect(truthy_matcher.failure_message_when_negated).to\
          match('Expected string not to improve_your_code of UncommunicativeVariableName, but it did')
      end
    end
    ...
    context 'when smell type is matching but smell details are not' do
      ...
      it 'sets the proper error message' do
        ...
        expected = <<-EOS.strip_heredoc
          Expected string to improve_your_code of DuplicateMethodCall (which it did) with smell details {:name=>"foo", :count=>15}, which it didn't.
      ...
      it 'sets the proper error message when negated' do
        ...
        expect(matcher.failure_message_when_negated).to\
          match('Expected string not to improve_your_code of DuplicateMethodCall with smell '\
  ...
  context 'with a smell that is disabled by default' do
    it 'enables the smell detector to match automatically' do
      default_config = ImproveYourCode::SmellDetectors::UnusedPrivateMethod.default_config
      src = 'class C; private; def foo; end; end'

      aggregate_failures do
        expect(default_config[ImproveYourCode::SmellConfiguration::ENABLED_KEY]).to be_falsy
