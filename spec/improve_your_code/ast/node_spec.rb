...
require_lib 'improve_your_code/ast/node'

RSpec.describe ImproveYourCode::AST::Node do
  describe '#format_to_ruby' do
    ...
    it 'gives the right result for if' do
      ...
      ast = ImproveYourCode::Source::SourceCode.from(source).syntax_tree
  ...
  describe '#line' do
    context 'with source from a file' do
      ...
      let(:ast) { ImproveYourCode::Source::SourceCode.from(file).syntax_tree }
    ...
    context 'with source from a string' do
      ...
      let(:ast) { ImproveYourCode::Source::SourceCode.from(source).syntax_tree }
