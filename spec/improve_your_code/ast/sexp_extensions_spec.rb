...
require_lib 'improve_your_code/ast/sexp_extensions'

RSpec.describe ImproveYourCode::AST::SexpExtensions::DefNode do
...
RSpec.describe ImproveYourCode::AST::SexpExtensions::CSendNode do
  let(:node) { ImproveYourCode::Source::SourceCode.from('1&.foo(true)').syntax_tree }

  it 'has a class including SendNode' do
    expect(node.class.included_modules).to include(ImproveYourCode::AST::SexpExtensions::SendNode)
  end
...
RSpec.describe ImproveYourCode::AST::SexpExtensions::DefsNode do
  context 'with no parameters' do
...
RSpec.describe ImproveYourCode::AST::SexpExtensions::LvarNode do
...
RSpec.describe ImproveYourCode::AST::SexpExtensions::ConstNode do
  describe '#name' do
...
RSpec.describe ImproveYourCode::AST::SexpExtensions::SendNode do
  context 'with no parameters' do
  ...
  context 'when it’s ‘new’ with a complex receiver' do
    let(:node) { ImproveYourCode::Source::SourceCode.from('(foo ? bar : baz).new').syntax_tree }
...
RSpec.describe ImproveYourCode::AST::SexpExtensions::BlockNode do
  context 'with no parameters' do
...
RSpec.describe ImproveYourCode::AST::SexpExtensions::LambdaNode do
...
RSpec.describe ImproveYourCode::AST::SexpExtensions::ModuleNode do
  context 'with a simple name' do
    let(:exp) do
      ImproveYourCode::Source::SourceCode.from('module Fred; end').syntax_tree
    end
  ...
  context 'with a scoped name' do
    let(:exp) do
      ImproveYourCode::Source::SourceCode.from('module Foo::Bar; end').syntax_tree
    end
...
RSpec.describe ImproveYourCode::AST::SexpExtensions::CasgnNode do
  describe '#defines_module?' do
    ...
    context 'with implicit receiver to new' do
      it 'does not define a module' do
        exp = ImproveYourCode::Source::SourceCode.from('Foo = Class.new(Bar)').syntax_tree
    ...
    context 'when assigning a lambda to a constant' do
      it 'does not define a module' do
        exp = ImproveYourCode::Source::SourceCode.from('C = ->{}').syntax_tree
    ...
    context 'when assigning a string to a constant' do
      it 'does not define a module' do
        exp = ImproveYourCode::Source::SourceCode.from('C = "hello"').syntax_tree
  ...
  describe '#superclass' do
    ...
    it 'returns nil in case of no class definition' do
      exp = ImproveYourCode::Source::SourceCode.from('Foo = 23').syntax_tree

      expect(exp.superclass).to be_nil
    end

    it 'returns nil in case of no superclass' do
      exp = ImproveYourCode::Source::SourceCode.from('Foo = Class.new').syntax_tree

      expect(exp.superclass).to be_nil
    end

    it 'returns nothing for a class definition using Struct.new' do
      exp = ImproveYourCode::Source::SourceCode.from('Foo = Struct.new("Bar")').syntax_tree

      expect(exp.superclass).to be_nil
    end

    it 'returns nothing for a constant assigned with a bare method call' do
      exp = ImproveYourCode::Source::SourceCode.from('Foo = foo("Bar")').syntax_tree
