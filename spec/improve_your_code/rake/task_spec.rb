...
require_lib 'improve_your_code/rake/task'

RSpec.describe ImproveYourCode::Rake::Task do
  describe '#source_files' do
    ...
    it 'is set to ENV["IMPROVE_YOUR_CODE_SRC"]' do
      begin
        ENV['IMPROVE_YOUR_CODE_SRC'] = '*.rb'
        task = described_class.new
        expect(task.source_files).to eq FileList['*.rb']
      ensure
        ENV['IMPROVE_YOUR_CODE_SRC'] = nil
      end
    end
  end
  ...
  describe '#source_files=' do
    ...
    it 'has no effect when ENV["IMPROVE_YOUR_CODE_SRC"] is set' do
      begin
        ENV['IMPROVE_YOUR_CODE_SRC'] = '*.rb'
        task = described_class.new do |it|
  ...
  describe '#command' do
    ...
    it 'includes a config file when set' do
      task.config_file = 'foo.improve_your_code'
      expect(task.send(:command)[1..2]).to eq ['-c', 'foo.improve_your_code']
    end
  end
end
