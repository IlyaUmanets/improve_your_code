...
require_lib 'improve_your_code/source/source_locator'

RSpec.describe ImproveYourCode::Source::SourceLocator do
  describe '#sources' do
    ...
    context 'with excluded paths' do
      let(:configuration) do
        test_configuration_for(CONFIG_PATH.join('with_excluded_paths.improve_your_code'))
      end

      let(:options) { instance_double('ImproveYourCode::CLI::Options', force_exclusion?: false) }
    ...
    context 'when passing "." or "./" as argument' do
      let(:expected_sources) do
        [Pathname.new('spec/spec_helper.rb'), Pathname.new('lib/improve_your_code.rb')]
      end
