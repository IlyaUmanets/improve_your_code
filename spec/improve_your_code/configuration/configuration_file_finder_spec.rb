...
RSpec.describe ImproveYourCode::Configuration::ConfigurationFileFinder do
  describe '.find' do
    ...
    it 'returns the file in current dir if path is not set' do
      ...
      expect(found).to eq(SAMPLES_PATH.join('exceptions.improve_your_code'))
    end
    ...
    it 'returns the file even if it’s just ‘.improve_your_code’' do
      ...
      expect(found).to eq(single_configuration_file_dir.join('.improve_your_code'))
    end
    ...
    it 'prefers the file in :current over one in :home' do
      ...
      file_in_current = SAMPLES_PATH.join('exceptions.improve_your_code')
    ...
    context 'with more than one configuration file' do
      ...
      it 'prints a message on STDERR' do
        expected_message = "Error: Found multiple configuration files 'regular.improve_your_code', 'todo.improve_your_code'"
        expect do
          begin
  ...
  describe '.load_from_file' do
    ...
    it 'loads the configuration from given file' do
      configuration = described_class.load_from_file(CONFIG_PATH.join('full_mask.improve_your_code'))
