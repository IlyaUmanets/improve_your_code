...
require_lib 'improve_your_code/configuration/app_configuration'
require_lib 'improve_your_code/configuration/directory_directives'
require_lib 'improve_your_code/configuration/default_directive'
require_lib 'improve_your_code/configuration/excluded_paths'

RSpec.describe ImproveYourCode::Configuration::AppConfiguration do
  describe 'factory methods' do
    ...
    let(:expected_default_directive) do
      { ImproveYourCode::SmellDetectors::IrresponsibleModule => { 'enabled' => false } }
    end

    let(:expected_directory_directives) do
      { SAMPLES_PATH.join('three_clean_files') =>
        { ImproveYourCode::SmellDetectors::UtilityFunction => { 'enabled' => false } } }
    end
    ...
    describe '#from_path' do
      let(:full_configuration_path) { CONFIG_PATH.join('full_configuration.improve_your_code') }
  ...
  describe '#directive_for' do
    context 'with multiple directory directives and no default directive present' do
      ...
      let(:baz_config)  { { ImproveYourCode::SmellDetectors::IrresponsibleModule => { enabled: false } } }
      let(:bang_config) { { ImproveYourCode::SmellDetectors::Attribute => { enabled: true } } }
    ...
    context 'with directory directive and default directive present' do
      ...
      let(:directory_config) { { ImproveYourCode::SmellDetectors::TooManyStatements => { max_statements: 8 } } }
      let(:directory_directives) { { directory => directory_config } }
      let(:default_directive) do
        {
          ImproveYourCode::SmellDetectors::IrresponsibleModule => { enabled: false },
          ImproveYourCode::SmellDetectors::TooManyStatements => { max_statements: 15 }
        }
      end
      ...
      it 'returns the directory directive with the default directive reverse-merged' do
        ...
        expect(actual[ImproveYourCode::SmellDetectors::IrresponsibleModule]).to be_truthy
        expect(actual[ImproveYourCode::SmellDetectors::TooManyStatements]).to be_truthy
        expect(actual[ImproveYourCode::SmellDetectors::TooManyStatements][:max_statements]).to eq(8)
      end
    end
    ...
    context 'with no directory directive but a default directive present' do
      ...
      let(:default_directive) { { ImproveYourCode::SmellDetectors::IrresponsibleModule => { enabled: false } } }
      let(:attribute_config) { { ImproveYourCode::SmellDetectors::Attribute => { enabled: false } } }
      let(:directory_directives) do
