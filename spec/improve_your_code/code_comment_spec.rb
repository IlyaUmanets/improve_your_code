...
require_lib 'improve_your_code/code_comment'

RSpec.describe ImproveYourCode::CodeComment do
  ...
  describe 'good comment config' do
    it 'parses hashed options' do
      comment = '# :improve_your_code:DuplicateMethodCall { enabled: false }'
    ...
    it 'parses multiple hashed options' do
      comment = <<-EOF
        # :improve_your_code:DuplicateMethodCall { enabled: false }
        # :improve_your_code:NestedIterators { enabled: true }
      EOF
    ...
    it 'parses multiple hashed options on the same line' do
      comment = <<-EOF
        #:improve_your_code:DuplicateMethodCall { enabled: false } and :improve_your_code:NestedIterators { enabled: true }
      EOF
      config = build(:code_comment, comment: comment).config
    ...
    it 'parses multiple unhashed options on the same line' do
      comment = '# :improve_your_code:DuplicateMethodCall and :improve_your_code:NestedIterators'
      config = build(:code_comment, comment: comment).config
    ...
    it 'disables the smell if no options are specifed' do
      comment = '# :improve_your_code:DuplicateMethodCall'
      config = build(:code_comment, comment: comment).config
    ...
    it 'ignores smells after a space' do
      config = build(:code_comment,
                     comment: '# :improve_your_code: DuplicateMethodCall').config
...
RSpec.describe ImproveYourCode::CodeComment::CodeCommentValidator do
  context 'when the comment contains an unknown detector name' do
    it 'raises BadDetectorInCommentError' do
      expect do
        build(:code_comment,
              comment: '# :improve_your_code:DoesNotExist')
      end.to raise_error(ImproveYourCode::Errors::BadDetectorInCommentError)
    end
  end
  ...
  context 'when the comment contains an unparsable detector configuration' do
    it 'raises GarbageDetectorConfigurationInCommentError' do
      expect do
        comment = '# :improve_your_code:UncommunicativeMethodName { thats: a: bad: config }'
        build(:code_comment, comment: comment)
      end.to raise_error(ImproveYourCode::Errors::GarbageDetectorConfigurationInCommentError)
    end
  end
  ...
  describe 'validating configuration keys' do
    context 'when basic options are mispelled' do
      it 'raises BadDetectorConfigurationKeyInCommentError' do
        expect do
          ...
          comment = '# :improve_your_code:UncommunicativeMethodName { exlude: alfa, nabled: true }'
          build(:code_comment, comment: comment)
        end.to raise_error(ImproveYourCode::Errors::BadDetectorConfigurationKeyInCommentError)
      end
    end
    ...
    context 'when basic options are not mispelled' do
      it 'does not raise' do
        expect do
          comment = '# :improve_your_code:UncommunicativeMethodName { exclude: alfa, enabled: true }'
          build(:code_comment, comment: comment)
      ...
      it 'does not raise on regexps' do
        expect do
          comment = '# :improve_your_code:UncommunicativeMethodName { exclude: !ruby/regexp /alfa/ }'
          build(:code_comment, comment: comment)
        end.not_to raise_error
      end
    end
    ...
    context 'when unknown custom options are specified' do
      it 'raises BadDetectorConfigurationKeyInCommentError' do
        expect do
          ...
          comment = '# :improve_your_code:DataClump { mx_copies: 4, mn_clump_size: 3 }'
          build(:code_comment, comment: comment)
        end.to raise_error(ImproveYourCode::Errors::BadDetectorConfigurationKeyInCommentError)
      end
    end
    ...
    context 'when valid custom options are specified' do
      it 'does not raise' do
        expect do
          comment = '# :improve_your_code:DataClump { max_copies: 4, min_clump_size: 3 }'
          build(:code_comment, comment: comment)
        end.not_to raise_error
      end
    end
