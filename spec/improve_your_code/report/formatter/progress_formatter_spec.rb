...
require_lib 'improve_your_code/report/formatter/progress_formatter'

RSpec.describe ImproveYourCode::Report::Formatter::ProgressFormatter::Dots do
  ...
  describe '#progress' do
    let(:clean_examiner) { instance_double(ImproveYourCode::Examiner, smelly?: false) }
    let(:smelly_examiner) { instance_double(ImproveYourCode::Examiner, smelly?: true) }
...
RSpec.describe ImproveYourCode::Report::Formatter::ProgressFormatter::Quiet do
  let(:sources_count) { 23 }
  let(:formatter) { described_class.new(sources_count) }
