...
require_lib 'improve_your_code/report/code_climate/code_climate_fingerprint'

RSpec.describe ImproveYourCode::Report::CodeClimateFingerprint do
  describe '#compute' do
    ...
    context 'when fingerprinting a warning with no parameters' do
      ...
      let(:warning) do
        build(:smell_warning,
              smell_detector: ImproveYourCode::SmellDetectors::UtilityFunction.new,
              context:        'alfa',
    ...
    context 'when the fingerprint should not be computed' do
      let(:warning) do
        build(:smell_warning,
              smell_detector: ImproveYourCode::SmellDetectors::ManualDispatch.new,
              context:        'Alfa#bravo',
    ...
    context 'when the smell warning has only identifying parameters' do
      let(:warning) do
        build(:smell_warning,
              smell_detector: ImproveYourCode::SmellDetectors::ClassVariable.new,
              context:        'Alfa',
    ...
    context 'when the smell warning has identifying and non-identifying parameters' do
      let(:warning) do
        build(:smell_warning,
              smell_detector: ImproveYourCode::SmellDetectors::DuplicateMethodCall.new,
              context:        "Alfa##{name}",
