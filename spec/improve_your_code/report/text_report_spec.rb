...
require_lib 'improve_your_code/report/text_report'
require_lib 'improve_your_code/report/formatter'
require 'rainbow'

RSpec.describe ImproveYourCode::Report::TextReport do
  let(:report_options) do
    {
      warning_formatter: ImproveYourCode::Report::Formatter::SimpleWarningFormatter.new,
      report_formatter: ImproveYourCode::Report::Formatter,
      heading_formatter: ImproveYourCode::Report::Formatter::QuietHeadingFormatter
    }
  end
  ...
  context 'with a single empty source' do
    before do
      instance.add_examiner ImproveYourCode::Examiner.new('')
    end
  ...
  context 'with non smelly files' do
    before do
      instance.add_examiner(ImproveYourCode::Examiner.new('def simple() puts "a" end'))
      instance.add_examiner(ImproveYourCode::Examiner.new('def simple() puts "a" end'))
    end
  ...
  context 'with a couple of smells' do
    before do
      instance.add_examiner(ImproveYourCode::Examiner.new('def simple(a) a[3] end'))
      instance.add_examiner(ImproveYourCode::Examiner.new('def simple(a) a[3] end'))
    end
