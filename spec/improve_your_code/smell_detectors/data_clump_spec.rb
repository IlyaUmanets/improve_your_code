...
require_lib 'improve_your_code/smell_detectors/data_clump'

RSpec.describe ImproveYourCode::SmellDetectors::DataClump do
  it 'reports the right values' do
    ...
    expect(src).to improve_your_code_of(:DataClump,
                           lines:      [2, 3, 4],
                           context:    'Alfa',
  ...
  it 'does count all occurences' do
    ...
    expect(src).
      to improve_your_code_of(:DataClump, lines: [2, 3, 4], parameters: ['echo', 'foxtrot']).
      and improve_your_code_of(:DataClump, lines: [6, 7, 8], parameters: ['juliett', 'kilo'])
  end
  ...
  %w(class module).each do |scope|
    it "does not report parameter sets < 2 for #{scope}" do
      ...
      expect(src).not_to improve_your_code_of(:DataClump)
    end
