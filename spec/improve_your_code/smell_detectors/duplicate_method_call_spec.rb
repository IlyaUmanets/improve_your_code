...
require_lib 'improve_your_code/smell_detectors/duplicate_method_call'

RSpec.describe ImproveYourCode::SmellDetectors::DuplicateMethodCall do
  it 'reports the right values' do
    ...
    expect(src).to improve_your_code_of(:DuplicateMethodCall,
                           lines:   [3, 4],
                           context: 'Alfa#bravo',
  ...
  it 'does count all occurences' do
    ...
    expect(src).
      to improve_your_code_of(:DuplicateMethodCall, lines: [3, 4], name: 'charlie.delta', count: 2).
      and improve_your_code_of(:DuplicateMethodCall, lines: [8, 9], name: 'foxtrot.golf', count: 2)
  end
  ...
  context 'with repeated method calls' do
    it 'reports repeated call to lvar' do
      ...
      expect(src).to improve_your_code_of(:DuplicateMethodCall, name: 'bravo.charlie')
    end

    it 'reports call parameters' do
      src = 'def alfa; @bravo.charlie(2, 3) + @bravo.charlie(2, 3); end'
      expect(src).to improve_your_code_of(:DuplicateMethodCall, name: '@bravo.charlie(2, 3)')
    end

    it 'reports nested calls' do
      src = 'def alfa; @bravo.charlie.delta + @bravo.charlie.delta; end'
      expect(src).
        to improve_your_code_of(:DuplicateMethodCall, name: '@bravo.charlie').
        and improve_your_code_of(:DuplicateMethodCall, name: '@bravo.charlie.delta')
    end

    it 'ignores calls to new' do
      src = 'def alfa; @bravo.new + @bravo.new; end'
      expect(src).not_to improve_your_code_of(:DuplicateMethodCall)
    end
  end
  ...
  context 'when allowing up to 3 calls' do
    let(:config) do
      { ImproveYourCode::SmellDetectors::DuplicateMethodCall::MAX_ALLOWED_CALLS_KEY => 3 }
    end

    it 'does not report double calls' do
      src = 'def alfa(bravo); bravo.charlie + bravo.charlie; end'
      expect(src).not_to improve_your_code_of(:DuplicateMethodCall).with_config(config)
    end
  ...
  context 'when allowing calls to some methods' do
    it 'does not report calls to some methods' do
      config = { ImproveYourCode::SmellDetectors::DuplicateMethodCall::ALLOW_CALLS_KEY => ['@bravo.charlie'] }
      src = 'def alfa; @bravo.charlie + @bravo.charlie; end'
      expect(src).not_to improve_your_code_of(:DuplicateMethodCall).with_config(config)
    end

    it 'reports calls to other methods' do
      config = { ImproveYourCode::SmellDetectors::DuplicateMethodCall::ALLOW_CALLS_KEY => ['@delta.charlie'] }
      src = 'def alfa; @bravo.charlie + @bravo.charlie; end'
      expect(src).to improve_your_code_of(:DuplicateMethodCall, name: '@bravo.charlie').with_config(config)
    end

    it 'does not report calls to methods specifed with a regular expression' do
      config = { ImproveYourCode::SmellDetectors::DuplicateMethodCall::ALLOW_CALLS_KEY => [/charlie/] }
      src = 'def alfa; puts @bravo.charlie; puts @bravo.charlie; end'
      expect(src).not_to improve_your_code_of(:DuplicateMethodCall, name: '@bravo.charlie').with_config(config)
    end
  end
end
