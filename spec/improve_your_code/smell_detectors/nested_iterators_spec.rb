...
require_lib 'improve_your_code/smell_detectors/nested_iterators'

RSpec.describe ImproveYourCode::SmellDetectors::NestedIterators do
  it 'reports the right values' do
    ...
    expect(src).to improve_your_code_of(:NestedIterators,
                           lines:   [3],
                           context: 'alfa',
  ...
  it 'does count all occurences' do
    ...
    expect(src).
      to improve_your_code_of(:NestedIterators, lines: [3], depth: 2).
      and improve_your_code_of(:NestedIterators, lines: [8], depth: 3)
  end

  it 'reports no smells with no iterators' do
    src = 'def alfa; end'
    expect(src).not_to improve_your_code_of(:NestedIterators)
  end
  ...
  it 'reports nesting inside iterator arguments' do
    ...
    expect(src).to improve_your_code_of(:NestedIterators, depth: 2)
  end
  ...
  it 'reports the deepest level of nesting only' do
    ...
    expect(src).
      to not_improve_your_code_of(:NestedIterators, depth: 2).
      and improve_your_code_of(:NestedIterators, depth: 3, lines: [4])
  end
  ...
  it 'reports all lines on which nested iterators occur' do
    ...
    expect(source).to improve_your_code_of(:NestedIterators, lines: [2, 3])
  end
  ...
  context 'when setting the allowed nesting depth to 3' do
    let(:config) do
      { ImproveYourCode::SmellDetectors::NestedIterators::MAX_ALLOWED_NESTING_KEY => 3 }
    end
    ...
    it 'does not report nested iterators 3 levels deep' do
      ...
      expect(src).not_to improve_your_code_of(:NestedIterators).with_config(config)
    end
    ...
    it 'reports nested iterators 4 levels deep' do
      ...
      expect(src).to improve_your_code_of(:NestedIterators).with_config(config)
    end
  end

  context 'when ignoring iterators' do
    let(:config) do
      { ImproveYourCode::SmellDetectors::NestedIterators::IGNORE_ITERATORS_KEY => ['ignore_me'] }
    end
    ...
    it 'reports nested iterators inside the ignored iterator' do
      ...
      expect(src).to improve_your_code_of(:NestedIterators, depth: 2).with_config(config)
    end
