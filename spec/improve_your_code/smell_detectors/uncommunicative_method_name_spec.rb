...
require_lib 'improve_your_code/smell_detectors/uncommunicative_method_name'

RSpec.describe ImproveYourCode::SmellDetectors::UncommunicativeMethodName do
  it 'reports the right values' do
    src = <<-EOS
      def m; end
    EOS

    expect(src).to improve_your_code_of(:UncommunicativeMethodName,
                           lines:   [1],
                           context: 'm',
  ...
  describe 'default configuration' do
    ...
    it 'does not report one-letter special characters' do
      ['+', '-', '/', '*'].each do |symbol|
        expect("def #{symbol}; end").not_to improve_your_code_of(:UncommunicativeMethodName)
      end
    end
  end
  ...
  describe '`accept` patterns' do
    ...
    it 'make smelly names pass via regex / strings given by list / literal' do
      [[/x/], /x/, ['x'], 'x'].each do |pattern|
        expect(source).not_to improve_your_code_of(:UncommunicativeMethodName).with_config('accept' => pattern)
      end
    end
  end
  ...
  describe '`reject` patterns' do
    ...
    it 'reject smelly names via regex / strings given by list / literal' do
      [[/alfa/], /alfa/, ['alfa'], 'alfa'].each do |pattern|
        expect(source).to improve_your_code_of(:UncommunicativeMethodName).with_config('reject' => pattern)
      end
    end
  end
