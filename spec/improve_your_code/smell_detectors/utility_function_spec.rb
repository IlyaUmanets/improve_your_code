...
require_lib 'improve_your_code/smell_detectors/utility_function'

RSpec.describe ImproveYourCode::SmellDetectors::UtilityFunction do
  it 'reports the right values' do
    ...
    expect(src).to improve_your_code_of(:UtilityFunction,
                           lines:   [1],
                           context: 'alfa',
  ...
  it 'counts a local call in a param initializer' do
    ...
    expect(src).not_to improve_your_code_of(:UtilityFunction)
  end
  ...
  it 'counts calls to self within a dstr' do
    ...
    expect(src).
      not_to improve_your_code_of(:UtilityFunction)
  end
  ...
  it 'reports a call to a constant' do
    ...
    expect(src).to improve_your_code_of(:UtilityFunction, context: 'simple')
  end
  ...
  context 'when examining singleton methods' do
    ...
    context 'when defined by using `module_function`' do
      ...
      it 'reports functions preceded by canceled modifier' do
        ...
        expect(src).to improve_your_code_of(:UtilityFunction, context: 'Alfa#bravo')
      end
  ...
  describe 'disabling UtilityFunction via configuration for non-public methods' do
    let(:config) do
      { ImproveYourCode::SmellDetectors::UtilityFunction::PUBLIC_METHODS_ONLY_KEY => true }
    end
    ...
    context 'when examining public methods' do
      it 'still reports UtilityFunction' do
        ...
        expect(src).to improve_your_code_of(:UtilityFunction, context: 'Alfa#bravo').with_config(config)
      end
    end
    ...
    context 'when examining private methods' do
      it 'does not report UtilityFunction' do
        ...
        expect(src).not_to improve_your_code_of(:UtilityFunction).with_config(config)
      end
