...
require_lib 'improve_your_code/smell_detectors/irresponsible_module'

RSpec.describe ImproveYourCode::SmellDetectors::IrresponsibleModule do
  it 'reports the right values' do
    ...
    expect(src).to improve_your_code_of(:IrresponsibleModule,
                           lines:   [1],
                           context: 'Alfa',
  ...
  it 'does count all occurences' do
    ...
    expect(src).
      to improve_your_code_of(:IrresponsibleModule, lines: [1], context: 'Alfa').
      and improve_your_code_of(:IrresponsibleModule, lines: [4], context: 'Alfa::Charlie')
  end
  ...
  %w(class module).each do |scope|
    ...
    it "does not report a #{scope} having a comment" do
      ...
      expect(src).not_to improve_your_code_of(:IrresponsibleModule)
    end
    ...
    it "does not report #{scope} used only as a namespace" do
      ...
      expect(src).not_to improve_your_code_of(:IrresponsibleModule, context: 'Alfa')
    end
    ...
    it "reports #{scope} that is used as a namespace but also has methods" do
      ...
      expect(src).to improve_your_code_of(:IrresponsibleModule, context: 'Alfa')
    end
    ...
    it "reports an irresponsible #{scope} defined through assignment" do
      src = <<-EOS
        ...
        #{scope} Alfa
          Bravo = Class.new Charlie # Only "class" is supposed to improve_your_code here.
        end
      EOS

      expect(src).to improve_your_code_of(:IrresponsibleModule, context: 'Alfa::Bravo')
    end
