...
require_lib 'improve_your_code/smell_detectors/uncommunicative_parameter_name'

RSpec.describe ImproveYourCode::SmellDetectors::UncommunicativeParameterName do
  it 'reports the right values' do
    ...
    expect(src).to improve_your_code_of(:UncommunicativeParameterName,
                           lines:   [1],
                           context: 'alfa',
  ...
  it 'does count all occurences' do
    ...
    expect(src).
      to improve_your_code_of(:UncommunicativeParameterName, lines: [1], name: 'x').
      and improve_your_code_of(:UncommunicativeParameterName, lines: [1], name: 'y')
  end
  ...
  { 'alfa.' => 'with a receiver',
    ...
    context "in a method definition #{description}" do
      it 'does not report two-letter parameter names' do
        ...
        expect(src).not_to improve_your_code_of(:UncommunicativeParameterName)
      end

      it 'reports names of the form "x2"' do
        src = "def #{host}bravo(x2) charlie(x2) end"
        expect(src).to improve_your_code_of(:UncommunicativeParameterName, name: 'x2')
      end

      it 'reports long name ending in a number' do
        src = "def #{host}bravo(param2) charlie(param2) end"
        expect(src).to improve_your_code_of(:UncommunicativeParameterName, name: 'param2')
      end
      ...
      it 'reports splat parameters' do
        expect("def #{host}bravo(*a); charlie(a); end").
          to improve_your_code_of(:UncommunicativeParameterName, name: 'a')
      end
      ...
      it 'reports used anonymous parameter' do
        ...
        expect(src).to improve_your_code_of(:UncommunicativeParameterName, name: '_')
      end

      it 'reports used parameters marked as unused' do
        src = "def #{host}bravo(_unused) charlie(_unused) end"
        expect(src).to improve_your_code_of(:UncommunicativeParameterName, name: '_unused')
      end

      it 'does not report anonymous splat' do
        expect("def #{host}bravo(*); end").not_to improve_your_code_of(:UncommunicativeParameterName)
      end
  ...
  describe '`accept` patterns' do
    ...
    it 'make smelly names pass via regex / strings given by list / literal' do
      [[/bar2/], /bar2/, ['bar2'], 'bar2'].each do |pattern|
        expect(source).not_to improve_your_code_of(:UncommunicativeParameterName).with_config('accept' => pattern)
      end
    end
  end
  ...
  describe '`reject` patterns' do
    ...
    it 'reject smelly names via regex / strings given by list / literal' do
      [[/bravo/], /bravo/, ['bravo'], 'bravo'].each do |pattern|
        expect(source).to improve_your_code_of(:UncommunicativeParameterName).with_config('reject' => pattern)
      end
    end
  end
