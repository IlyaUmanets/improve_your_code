...
require_lib 'improve_your_code/smell_detectors/uncommunicative_variable_name'

RSpec.describe ImproveYourCode::SmellDetectors::UncommunicativeVariableName do
  it 'reports the right values' do
    ...
    expect(src).to improve_your_code_of(:UncommunicativeVariableName,
                           lines:   [2],
                           context: 'alfa',
  ...
  it 'does count all occurences' do
    ...
    expect(src).to improve_your_code_of(:UncommunicativeVariableName, lines: [2], name: 'x').
      and improve_your_code_of(:UncommunicativeVariableName, lines: [3], name: 'y')
  end
  ...
  context 'when examining instance variables' do
    it 'does not report use of one-letter names' do
      ...
      expect(src).not_to improve_your_code_of(:UncommunicativeVariableName)
    end

    it 'reports one-letter names in assignment' do
      src = 'class Alfa; def bravo(charlie) @x = charlie; end; end'
      expect(src).to improve_your_code_of(:UncommunicativeVariableName, name: '@x')
    end
  end
  ...
  context 'when examining local variables' do
    ...
    it 'reports names ending with a digit' do
      ...
      expect(src).to improve_your_code_of(:UncommunicativeVariableName, name: 'var123')
    end

    it 'reports camelcased names' do
      src = 'def alfa; bravoCharlie = delta(); end'
      expect(src).to improve_your_code_of(:UncommunicativeVariableName, name: 'bravoCharlie')
    end
    ...
    it "reports bad names starting with '_'" do
      ...
      expect(src).to improve_your_code_of(:UncommunicativeVariableName, name: '_bravoCharlie_42')
    end
  end
  ...
  context 'when examining block parameters' do
    it 'reports all relevant block parameters' do
      ...
      expect(src).to improve_your_code_of(:UncommunicativeVariableName, name: 'x').
        and improve_your_code_of(:UncommunicativeVariableName, name: 'y')
    end
    ...
    it 'reports splatted block parameters correctly' do
      ...
      expect(src).to improve_your_code_of(:UncommunicativeVariableName, name: 'y')
    end
    ...
    it 'reports deeply nested block parameters' do
      ...
      expect(src).to improve_your_code_of(:UncommunicativeVariableName, name: 'x').
        and improve_your_code_of(:UncommunicativeVariableName, name: 'y').
        and improve_your_code_of(:UncommunicativeVariableName, name: 'z')
    end
  ...
  describe '`accept` patterns' do
    ...
    it 'make smelly names pass via regex / strings given by list / literal' do
      [[/bravo2/], /bravo2/, ['bravo2'], 'bravo2'].each do |pattern|
        expect(src).to improve_your_code_of(:UncommunicativeVariableName).
          and not_improve_your_code_of(:UncommunicativeVariableName).with_config('accept' => pattern)
      end
    end
  end
  ...
  describe '`reject` patterns' do
    ...
    it 'reject smelly names via regex / strings given by list / literal' do
      [[/foobar/], /foobar/, ['foobar'], 'foobar'].each do |pattern|
        expect(src).to not_improve_your_code_of(:UncommunicativeVariableName).
          and improve_your_code_of(:UncommunicativeVariableName).with_config('reject' => pattern)
      end
    end
  end
