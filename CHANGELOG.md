...
* (mvz) Pick up config file in improve_your_code matcher
* (mvz) Let IMPROVE_YOUR_CODE_SRC override set values in rake task
...
* (mvz) Raise ImproveYourCode-specific error on parse errors
...
* (troessner) Make ImproveYourCode more resilient on incomprehensible source.
...
* (mvz) Implement and use ShouldImproveYourCodeOf#with_config
* (mvz) Automatically enable smell in improve_your_code_of matcher
...
* First pre-release for ImproveYourCode 4.
...
* (mvz) Remove smell category concept:
  ...
  - The smell category can no longer be used to select smells in the improve_your_code_of
    matcher.
* (chastell) Make HTML report print to stdout like the other reports
* (troessner) Drop support for CRuby 2.0
* (mvz) Speed up improve_your_code_of matcher by only running the given smell
...
* Add CodeClimate Docker integration. This will allow users to deduct their own docker image
  ...
  Furthermore this will enable users to run `ImproveYourCode` locally in combination with `codeclimate cli`.
...
* (tansaku + mvz) Allow matches in improve_your_code_of for message, lines, context and source.
...
* (troessner) Fix `improve_your_code_of` not checking `smell_details`
...
* (troessner) Fix ImproveYourCode descending into hidden directories
...
* (mvz) Ensure require 'improve_your_code' is enough to use ImproveYourCode's classes
* (mvz) Pick config file that comes first alphabetically
* (mvz) Separate FeatureEnvy and UtilityFunction

## 2.0.4 (2015-04-07)

* (chastell)  Recognise .improve_your_code as a valid configuration file
...
* (chastell) Escape Regexp-like Strings on CodeContext matches (Bug https://github.com/troessner/improve_your_code/pull/397)
...
* (troessner) Revise configuration handling:
  Now there are 3 ways of passing ImproveYourCode a configuration file:
  - Using the cli "-c" switch
  - Having a file ending with .improve_your_code either in your current working directory or in a parent directory (more on that later)
  - Having a file ending with .improve_your_code in your HOME directory

  The order in which ImproveYourCode tries to find such a configuration file is exactly
  like above: First ImproveYourCode checks if we have given it a configuration file
  ...
  directory. And lastly, it checks your HOME directory. As soon as ImproveYourCode detects a
  configuration file it stops searching immediately, meaning that from ImproveYourCode's
  point of view there exists one configuration file and one configuration only
  regardless of how many ".improve_your_code" files you might have on your filesystem.
...
* (gilles-leblanc) Add color to ImproveYourCode's output
...
* (gilles-leblanc) Added line numbers by default when you run the improve_your_code command
...
* (mvz) Use ripper_ruby_parser on Ruby 1.9.3 and up (thus making ImproveYourCode able
...
* Smell detectors can be configured or disabled in code comments
  * Comment with `:improve_your_code:smell_name` disables the named smell for a class, module or method
  * Comment with `:improve_your_code:smell_name:{...}` for more detailed configuration
* Additional config file(s) can be specified:
  * on the command-line using -c
  * via ImproveYourCode::Rake::Task in the rakefile
...
* Now require 'improve_your_code/rake/task' to use the rake task
* Now require 'improve_your_code/spec' to use the Rspec matchers
...
See http://wiki.github.com/kevinrutherford/improve_your_code for further details.
...
* Removed support for MyClass.should_not improve_your_code due to ParseTree EOL
...
See http://wiki.github.com/kevinrutherford/improve_your_code for details

## 1.2 2009-09-20

### Major Changes
* ImproveYourCode passes all its tests under Ruby 1.8.6, 1.8.7 and 1.9.1 (fixed #16)
...
* ImproveYourCode's output reports are now formatted differently:
  * ImproveYourCode is no longer silent about smell-free source code
...
* The smells masked by `*.improve_your_code` config files can now be seen:
  * The header for each source file now counts masked smells
  * The --show-all (-a) option shows masked warnings in the report
* The spec matchers are now accessed by requiring 'improve_your_code/adapters/spec'

### Minor Changes
* ImproveYourCode's RDoc is now hosted at http://rdoc.info/projects/kevinrutherford/improve_your_code
...
* Several changes to the LongMethod counting algorithm:
  ...
  * See http://wiki.github.com/kevinrutherford/improve_your_code/long-method for details
...
* 'MyClass.should_not improve_your_code' now only possible if ParseTree gem installed
...
### Minor enhancements
* Now possible to write 'MyClass.should_not improve_your_code' (fixed #33)

### Fixes
* Now counts attr assignments ([]= etc) in feature envy calculations
* Doesn't attempt to find `*.improve_your_code` files when reading code from stdin
...
### Major enhancements
* Use `*.improve_your_code` files in source tree to configure ImproveYourCode's behaviour
* Added -f option to configure report format
* --sort_order replaced by -f, -c and -s
* Matchers provided for rspec; eg. foo.should_not improve_your_code
...
* ImproveYourCode now only checks sources passed on the command line
* Code snippets can be supplied on the commandline
* Added headings and warnings count when smells in multiple files
* Added ImproveYourCode::RakeTask to run ImproveYourCode from rakefiles
...
* Tweaks:
  ...
  * Added integration tests that run ImproveYourCode on a couple of gems
